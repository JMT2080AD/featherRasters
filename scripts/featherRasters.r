...
featherRasters <- function(rast1, rast2, featherDist){
    ...
    ## merge rasters
    rastNewPoly <- gUnion(rast1Edge[["poly"]], rast2Edge[["poly"]])
    rastNew <- merge(rast1, rast2)
    rast1   <- resample(rast1, rastNew)
    rast2   <- resample(rast2, rastNew)

    extractFun <- function(rast, rastNewPoly){
        require(raster)
        rastCells <- data.frame(extract(rast, rastNewPoly, cellnumbers = T)[[1]])
        rast[rastCells$cell[is.na(rastCells$value)]] <- 0
        return(rast)
    }

    cl <- makeCluster(2)
    rastList <- parLapply(cl, list(rast1, rast2), extractFun, rastNewPoly = rastNewPoly)
    stopCluster(cl)
    ...
    return(list(rast1, rast2, rastNew))
